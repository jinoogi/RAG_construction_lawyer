''' 법령 본문과 하위 법령을 구조화해서 반환하고, 저장도 함'''
        ...
        if checkbox != None:
            ...
            for link in div_soup.select("a[onclick^='javascript:fncLawPop']"):
                ...
                if '제' in link.text and '조' in link.text:  # 텍스트 필터링
                    ...
                    time.sleep(10)  # 하위 법조항 페이지 로드 대기}
                    driver.switch_to.window(driver.window_handles[1]) # 하위법조항 창으로 이동
                    # 하위법조항 내용추출
                    ...
                    driver.switch_to.window(driver.window_handles[0]) # 메인법조항창으로 복귀
...
with open("crawl_result.pkl", "wb") as file:
    pickle.dump((main_laws, sub_laws), file)